...
# OB-HAVO FUNKSIYASI (API orqali - Kunduzi va Kechqurun)
def get_weather_data(city):
    try:
        url = f"http://api.openweathermap.org/data/2.5/forecast?q={city}&appid=b713020054700d98192801e0e8e97495&units=metric&lang=uz"
        r = requests.get(url, timeout=10).json()
        d_temp = r['list'][0]['main']['temp_max']
        n_temp = r['list'][8]['main']['temp_min']
        desc = r['list'][0]['weather'][0]['description']
...
def handle_menu(m):
    if m.text == "📰 So'nggi Yangiliklar":
        try:
            bot.send_message(m.chat.id, "⌛️ Yangiliklar yuklanmoqda...")
        ...
        bot.send_message(m.chat.id, "🌤 12 viloyat va Qoraqalpog'istondan birini tanlang:", reply_markup=kb)
        ...
        bot.send_message(m.chat.id, "🎥 Janrni tanlang (10 tadan film):", reply_markup=kb)

@bot.callback_query_handler(func=lambda call: True)
def callback_all(call):
    if call.data.startswith("w_"):
        bot.send_message(call.message.chat.id, get_weather_data(call.data[2:]), parse_mode="Markdown")
    elif call.data.startswith("k_"):
        res = "🎬 **Siz tanlagan janr bo'yicha TOP 10 film:**\n\n"
        res += "\n".join([f"{i}. [Kino linki {i}](https://uzmovi.com/search?q=kino)" for i in range(1, 11)])
