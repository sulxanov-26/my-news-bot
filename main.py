...
# 2. OB-HAVO FUNKSIYASI (API)
def get_weather(city_name):
    try:
        url = f"http://api.openweathermap.org/data/2.5/weather?q={city_name}&appid=b713020054700d98192801e0e8e97495&units=metric&lang=uz"
        r = requests.get(url, timeout=10).json()
        temp = r['main']['temp']
        return f"📍 {city_name.capitalize()}: {int(temp)}°C"
    except:
        return "⚠️ Ob-havo ma'lumotini olib bo'lmadi."
...
def start(m):
    ...
    kb.add("📰 Kun.uz Yangiliklar", "💰 Valyuta Kursi", "🌤 Ob-havo", "🎬 Kinolar")
...
def main_menu(m):
    if m.text == "📰 Kun.uz Yangiliklar":
        ...
        except: bot.send_message(m.chat.id, "⚠️ Yangiliklarni yuklab bo'lmadi.")

    elif m.text == "💰 Valyuta Kursi":
        try:
            r = requests.get("https://cbu.uz/uz/arkhiv-kursov-valyut/json/").json()
            res = "💰 **Markaziy Bank kursi:**\n\n" + "\n".join([f"🔹 1 {i['Ccy']} = {i['Rate']} so'm" for i in r[:4]])
