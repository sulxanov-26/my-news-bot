...
def get_kun_uz():
    try:
        ...
        root = ET.fromstring(r.content)
        return [f"🔵 {i.find('title').text}\n🔗 [Xabarni ochish]({i.find('link').text})" for i in root.findall('.//item')[:10]]
...
def start(message):
    ...
    welcome_text = (
        ...
        "Sizning shaxsiy yordamchingiz — **Zamondosh** botiga xush kelibsiz! 🚀\n\n"
        "Marhamat, quyidagi menyudan o'zingizga kerakli bo'limni tanlang: 👇"
    )
...
def handle_text(message):
    if message.text == "🔵 Kun.uz":
        bot.send_message(message.chat.id, "📢 **Kun.uz: Oxirgi 10 ta yangilik:**\n\n" + "\n\n".join(get_kun_uz()), parse_mode="Markdown")
    elif message.text == "🏆 Sport/Futbol":
        bot.send_message(message.chat.id, "⏳ Yangiliklar yuklanmoqda...")
        news = get_sport_news()
        bot.send_message(message.chat.id, "\n\n".join(news) if news else "⚠️ Xabar topilmadi.", parse_mode="Markdown", disable_web_page_preview=True)
    elif message.text == "💰 Valyuta":
        r = requests.get("https://cbu.uz/uz/arkhiv-kursov-valyut/json/").json()
        res = "💰 **Valyuta kursi (CBU):**\n\n" + "\n".join([f"🔹 1 {i['Ccy']} = {i['Rate']} so'm" for i in r[:10]])
        bot.send_message(message.chat.id, res)
    elif message.text == "🎬 Kinolar":
        bot.send_message(message.chat.id, "🎥 Janrni tanlang (Ko'rish uchun nomi ustiga bosing):", reply_markup=movie_genres())
...
def callback_inline(call):
    ...
    if call.data in weather_info:
        ...
        res = movies.get(call.data, "🎬 Tez kunda barcha linklar qo'shiladi!")
